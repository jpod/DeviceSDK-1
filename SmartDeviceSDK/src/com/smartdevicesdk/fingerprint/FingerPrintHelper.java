...
public class FingerPrintHelper {
	...
	private void openSerialPort(String deviceStr, int baudrate) {
		if (mSerialPort != null) {
			mSerialPort.closePort();
			mSerialPort = null;
		}
		mSerialPort = new SerialPort(deviceStr, baudrate);
		mSerialPort.setOnserialportDataReceived(new SerialPortDataReceived() {
			...
			public void onDataReceivedListener(byte[] buffer, int size) {
				...
				Log.i(TAG, "onDataReceivedListener:size" + size + ",data:"
						+ byte2HexStr(buffer));
				datalen += size;
	...
	public byte[] SendPackage(byte[] buffer) {
		...
		while (System.currentTimeMillis() - stime < 5000) {
			...
			if (samecount > 1000) {
				...
				Log.i(TAG, "SendPackage onDataReceivedListener size:"
						+ bt.length);
				return bt;
			}
		}
