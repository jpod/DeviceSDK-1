			...
			while (true) {
				...
				while (cursor.moveToNext()) {
					byte[] buffer = cursor.getBlob(cursor
							.getColumnIndex("bytearray"));
					Log.i(TAG, "WriteThread,数据大小："+buffer.length);
					if (mSerialPort.Write(buffer)) {
						dbhelper.delete(cursor.getInt(cursor
								.getColumnIndex("id")));
					}
					if (buffer.length > 1000) {
						...
						try {
							// 根据数据包大小延时软流控
							Thread.sleep((buffer.length / 1000) * 100);
						...
						try {
							String str="\r\n";
//							mSerialPort.Write(str.getBytes());
							// 根据数据包大小延时软流控
							Thread.sleep(buffer.length*5);
						} catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
	...
	private boolean getBufferState(final int sleepTime) {
		...
		for (int i = 0; i < sleepTime / 10; i++) {
			...
			if (PrintService.printState == 0) {
				PrintService.getState = false;
				Log.i(TAG, "getBufferState:打印机空闲,查询次数：" + i);
				return true;
			} else {
				Log.i(TAG, "getBufferState:打印机忙碌,查询次数：" + i);
			}
			try {
				Thread.sleep(10);
			} catch (InterruptedException e) {
