...
public class DeviceAll implements DeviceInfo {
	...
	public List<HashMap<String, String>> getFunctionList() {
		...
		map.put("id", "printer");
		map.put("name", "打印机 printer");
		data.add(map);

		map = new HashMap<String, String>();
		map.put("id", "scanner");
		map.put("name", "扫描-扫描模块 scanner");
		data.add(map);

		map = new HashMap<String, String>();
		map.put("id", "camerascanner");
		map.put("name", "扫描-摄像头 camera scan");
		data.add(map);

		map = new HashMap<String, String>();
		map.put("id", "psam");
		map.put("name", "接触式PSAM卡 PSAM");
		data.add(map);

		map = new HashMap<String, String>();
		map.put("id", "magneticcard");
		map.put("name", "磁条卡(I2C) magnetic card");
		data.add(map);

		map = new HashMap<String, String>();
		map.put("id", "fingerprint");
		map.put("name", "指纹 fingerprint");
		data.add(map);
		
		map = new HashMap<String, String>();
		map.put("id", "cscreen");
		map.put("name", "客显屏幕 Customer Screen");
		data.add(map);

		map = new HashMap<String, String>();
		map.put("id", "idcard");
		map.put("name", "二代身份证-公安部模块 idcard");
		data.add(map);
		
		map = new HashMap<String, String>();
		map.put("id", "serialport");
		map.put("name", "串口工具 serialport tools");
		data.add(map);
	...
	public String getPrinterSerialport() {
		String serialport_name="/dev/ttyUSB1";
		return serialport_name;
	}
	@Override
	public int getPrinterBaudrate() {
		int serialport_baudrate=460800;
